...
<body class="childrenBody">
	<form class="layui-form" action="<%=path%>/news/xgbc.action" method="post" target="_parent">
		<div class="layui-form-item">
	
			<label class="layui-form-label">选择新闻类别</label>
			<select style="width: 100px" name="typeid">
			<option value="${ss.smt.id}">${ss.smt.newsname}</option>
		...
		<div class="layui-form-item">
			<label class="layui-form-label">修改新闻标题</label>
			<div class="layui-input-block">
				<input type="text" name="titile" value="${ss.titile }" class="layui-input newsName" lay-verify="required" placeholder="请输入需要的新闻标题">
				<input type="hidden" name="tid" value="${ss.id }">
			</div>
		</div>
		<div class="layui-form-item">
			<label class="layui-form-label">修改新闻内容</label>
			<div class="layui-input-block">
				<input type="text" name="content" value="${ss.content }"  class="layui-input newsName" lay-verify="required" placeholder="请输入需要的新闻内容">
			</div>
		</div>
		
		
		<div class="layui-form-item">
			<label class="layui-form-label">修改新闻类别</label>
			<select style="width: 100px" name="type">
				<option value="0" <c:if test="${ss.smt.type==0 }">selected</c:if>>列表</option>
				<option value="1" <c:if test="${ss.smt.type==1 }">selected</c:if>>文章</option>
			</select>
		</div>
