layui.config({
	...
	//$(window).one("resize",function(){
		...
		$(".newsAdd_btn1").click(function(){
			var index = layui.layer.open({
				title : "添加新闻内容",
				type : 2,
				area:['500px','300px'],
				content : "tjxwmr.action",
				success : function(layero, index){
					setTimeout(function(){
						layui.layer.tips('点击此处返回商品列表', '.layui-layer-setwin .layui-layer-close', {
							tips: 3
		...
		$(".news_edit").click(function(){
			...
			var index = layui.layer.open({
				...
				type : 2,
				area:['500px','300px'],
				content : "xgxw.action?id="+orgid,
				success : function(layero, index){
					setTimeout(function(){
						layui.layer.tips('点击此处返回快递列表', '.layui-layer-setwin .layui-layer-close', {
							tips: 3
						});
					},500)
				}
	...
	$("body").on("click",".news_del",function(){  //删除
		...
		layer.confirm('确定删除此信息？',{icon:3, title:'提示信息'},function(index){
			document.location.href="news/delete1.action?id="+orgid;
			layer.close(index);
		});
	})
